...
import com.leo.events.model.Comment;
            ...
            holder.tvShare.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (AccessibilityUtil.isAccessibilitySettingsOn(holder.itemView.getContext())) {
                        if (!ShareWindow.isShowWindow()) {
                            ShareWindow.toStartWindow();
                            return;
                        }
                        ShareWindow.get().show();
                    }

                    String s = goods.title + "\n" + goods.text + "\n";
                    if (goods.comments != null && !goods.comments.isEmpty()) {
                        s = s + "网友评论: \n";
                        for (Comment comment : goods.comments) {
                            if (comment.creator != null && !TextUtils.isEmpty(comment.creator.username)) {
                                if (TextUtils.isEmpty(comment.parent)) {
                                    s = s + comment.creator.username + ":";
                                } else {
                                    s = s + comment.creator.username + "回复" + comment.parent + ":";
                                }
                            }
                            s = s + comment.text;
                        }
                    ...
                    ShareUtil.shareImage(holder.itemView.getContext(), SharePlatform.WX_TIMELINE, IMAGE_BASE + goods.smallImage, new ShareListener() {
                        ...
                        public void shareFailure(Exception e) {
                            Toast.makeText(holder.itemView.getContext(), "分享失败", Toast.LENGTH_LONG).show();
                            AccessibilityEventManaer.get().dispose();
                            close();
                        }
